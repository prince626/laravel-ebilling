...
class subscriptionAction extends Controller
{
    function users_editkey(Request $req, $subsId)
    {
        try {
            ...
            //     $findAction = activatceaction::where('subs_id', $user->subs_id)
            //         ->latest('created_at')
            //         ->first();
            ...
            UpdateHelper::notification($req, 'isUser', $user, "Your Subscription has been " . ($user->activationStatus ? 'Activated' : 'Deactivated') . "by Administrator", 'success');
